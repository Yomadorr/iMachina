...
<style>
	...
	.classEvaluationTextClear { background: #ffffff; border: 1px solid; _width: 900px; background: #cc000000; }
		.classEvaluationTextLine { _border-bottom: 1px solid black; }
			...
			.classEvaluationTextLineComment { display:inline; font-size: 12px; color: #777777; _border-bottom: 1px solid; padding:0px; border:0px;}
</style>
  ...
  */
  ...
  var arrComments=new Array();
	   ...
	   var Comment = function( )
	   {
			...
			// updateTo
			// todo: pack into two depending functions
			this.updateToWithoutId = function( copyCommentObject )
			{
				// this.id=copyCommentObject.id;
...
// todo: right place in script
		...
		function actionAddOrUpdateInspector()
		{
			...
			if (actualComment.id==-1) 
			{
				...
				addThisComment.updateToWithoutId(actualComment);
			...
			// update object
			{
				...
				commentObj.updateToWithoutId(actualComment);
			...
			*/
		    ...
		    $('#commentInspectorFormCommandLine').keyup(function(event) {
				...
				if (commentObj!=null)
				{
					// update to object
					// version 1.0
					// alert("actualComment.id "+actualComment.id);
					actualComment.updateToWithoutId(commentObj);
